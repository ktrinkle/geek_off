...
<?php echo form_open('emp_2', array('id'=>'emp2'), array('event' => $event)); ?>
<div class="uk-grid" uk-grid>
	...
	$j = $elementcount - 1; 
	echo "<div class='uk-text-center bigbtngrn uk-card-small uk-card-body'>";
	if ($j <= $elementcount) {
		$pyr  = $questions->row($j);
		echo form_checkbox(array('name'=>'q' . $pyr->question_no, 'id'=>'q' . $pyr->question_no, 'value' => $pyr->ptsposs, 'checked' => FALSE, 'style' =>'class:bigbtngrn'));
		echo form_label('Bonus&nbsp;1', 'q' . $pyr->question_no, array('class' => 'bigbtngrnlabel'));
	}
	echo "</div>";		
	$j = $elementcount; 
	echo "<div class='uk-text-center bigbtngrn uk-card-small uk-card-body'>";
	if ($j <= $elementcount) {
		$pyr  = $questions->row($j);
		echo form_checkbox(array('name'=>'q' . $pyr->question_no, 'id'=>'q' . $pyr->question_no, 'value' => $pyr->ptsposs, 'checked' => FALSE, 'style' =>'class:bigbtngrn'));
		echo form_label('Bonus&nbsp;2', 'q' . $pyr->question_no, array('class' => 'bigbtngrnlabel'));
	}
	echo "</div>";		
	?>
</div>
...
<?php
	...
	//The new board style for password
		$newrows = ceil(($elementcount-2)/2); //account for 0 based
		for($k=0; $k<$newrows; $k++) {
			echo "<div class='uk-grid uk-child-width-1-2@s uk-text-center' uk-grid>";
			for($m=0;$m<2;$m++) {
				if ($k+($m*$newrows) <= $elementcount-2) {
					$pyr  = $questions->row($k+($m*$newrows));
					echo "<div class='bigbtnred uk-card-small uk-card-body'>";
					echo form_checkbox(array('name'=>'q' . $pyr->question_no, 'id'=>'q' . $pyr->question_no, 'value' => $pyr->ptsposs, 'checked' => FALSE, 'style' =>'class:bigbtnred'));
					echo form_label(($pyr->question_no-200), 'q' . $pyr->question_no, array('class' => 'bigbtnredlabel'));
				}
				echo "</div>\n";
			}
			echo "</div>\n";
...
<?php echo form_close();?>
    	<?php
		echo form_open('finalize_round',array('id'=>'finalize'),array('round' => '2',
		 'event' => $event, 'next_page' => 'emp_3')) . form_submit('finalize', 'Finalize round 2',array('class' => 'uk-button uk-button-secondary')) . form_close();
?>
<input type="hidden" id="round" name="round" class="round_no" value="2" />&nbsp;
